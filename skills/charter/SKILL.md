...
- Use `document-traceability` to stamp canonical provenance with `source_artifacts: {}` because charter is the root artifact in its branch.
...
- The artifact must record canonical provenance through `document-traceability` with `source_artifacts: {}`.
...
- stamping deterministic provenance
...
5. Stamp canonical provenance with `source_artifacts: {}`.
6. Keep the artifact on the approved scope baseline; move user-visible behaviors, detailed requirements, and design ideas out of the document.
...
9. Deliver the draft and request approval before user stories, requirements, or technical design proceed.
...
- If provenance is omitted because charter has no source artifacts, the root of the artifact branch still becomes untraceable. Keep the canonical provenance block even on the root artifact.
...
- canonical provenance is present
